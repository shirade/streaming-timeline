...
var domain =  process.env['TWITTER_CLIENT_DOMAIN'] || 'localhost';
...
io.on('connect', function(socket){
  ...
  redisStore.get(sid, function (error, session) {
    if (error) {
      ...
      stream.on('tweet', function (tweet) {
        ...
        db.insertTweet(trimmedTweet, passportSession.id, function (error, user) {
          if (error) console.error(error);
        });
      }).on('delete', function (msg) {
        io.to(socket.id).emit('delete tweet', msg.delete.status);
        db.removeTweet(msg.delete.status, function (error, user) {
          if (error) console.error(error);
        });
      });
      ...
      twit.get('statuses/home_timeline', {
        ...
        io.to(socket.id).emit('init', timeline.slice(0, 5));
        db.upsertUser({
  ...
  socket.on('supplemental tweet', function () {
    db.getUser(passportSession.id, function (error, user) {
...
// オブジェクト作成は楽にできたが，その後は楽できるのか？
function getSID (socket) {
  return cookie.parse(socket.handshake.headers.cookie)['connect.sid'].split('.')[0].slice(2);
};
